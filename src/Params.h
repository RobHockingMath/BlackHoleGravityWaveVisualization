...
struct Params {
    // Image / output.
    int width = 640*3;//420;
    int height = 640*3;//560;
    int numThreads = 0;        // 0 means auto.
    int samplesPerPixel = 9;   // 1 = no AA. 4/9/16 give stratified supersampling.
    ...
    bool gwpvAdaptiveEnabled = false;
    ...
    // CUDA metric-lensing knobs. With numericalMetricEnabled=false, the CUDA
    // lensed path uses the old Majumdar-Papapetrou placeholder. With
    // numericalMetricEnabled=true, it loads a precomputed NearFieldExtract
    // metric_4d_plus_christoffel_v1 snapshot and traces every pixel through one
    // continuous piecewise geodesic: numerical acceleration inside safe sampled
    // boxes, zero acceleration outside them.
    double cudaMetricStep = 0.1;       // fixed RK4 geometry step
    double cudaMetricColorStep = 0.1;  // fixed volume/color substep along each curved segment
    double cudaDoublePixelRadiusFraction = 0.10; // MP-only central double-precision region
    int cudaMetricMaxSteps = 20000;     // safety cap per ray

    // NearFieldExtract v7 numerical metric prototype.
    bool numericalMetricEnabled = true;
    std::string numericalMetricSnapshotStem =
        "/mnt/nvme0/home/sasuke/EinsteinToolkit/Cactus/N6_outsideAH_t30_extract/nearfield_christoffel_fd2_N6_outsideAH_t30_000005";
    std::string numericalMetricHorizonPath =
        "/mnt/nvme0/home/sasuke/EinsteinToolkit/Cactus/N6_outsideAH_t30_extract/nearfield_horizons_N6_outsideAH_t30.txt";
    int numericalMetricMaxLayer = 6;        // render tests: 0, 1, 2, 3, 4
    // Reject numerical-grid samples too close to sampled-box faces.  The FD2
    // Christoffel postprocessor writes NaN Christoffels on the one-cell derivative
    // stencil shell, so B=1 is the minimum safe value.  Use 2 for a conservative
    // comparison if needed.
    double numericalMetricBoundaryBufferCells = 2.0;
    // Fail-fast debug thresholds for interpolated numerical data.  These are
    // deliberately loose garbage detectors, not physical validity bounds.
    bool numericalMetricFailFast = true;
    // Diagnostic color-mask mode for the numerical full-ray path.  When true,
    // the CUDA renderer ignores the usual panorama/volume color and outputs:
    //   red       = captured by AH1
    ...
    //   orange    = hit the RK step cap
    //   white     = numerical failure if fail-fast is disabled
    bool numericalMetricDebugMask = false;
    double numericalMetricMetricFailAbs = 1.0e4;
    double numericalMetricGammaFailAbs = 1.0e4;
    double numericalMetricAccelFailAbs = 1.0e5;
    // Retained for old experiments; ignored by the precomputed-Christoffel CUDA path.
    bool numericalMetricUseTimeDerivatives = true;
    double numericalMetricHorizonSafetyFactor = 1.1;
