...
constexpr double ANIM_START_TIME = 30.012;//700-1*192-1*100;//1350.0-150.0; // 250 becomes 500
constexpr double ANIM_END_TIME = 30.012+600;//700+600-1*192-1*100;//1600.0+100.0;
const std::string FRAME_DIR = "frames_cuda_r53_colorfalloff11";
...
constexpr double CAMERA_RADIUS_X = 220.0*0.125;
constexpr double CAMERA_RADIUS_Y = -320.0*0.125;
constexpr double CAMERA_Z = 180.0*0.125;
...
void printCudaFrameTiming(const Params& frameParams, double seconds) {
    ...
    std::cerr << "CUDA frame render time: " << seconds << " s"
...
int main() {
    try {
        ...
        // CUDA renderer: fixed-step gwpv_peaks.  metricLensingEnabled is now
        // controlled by Params.h: false gives the original straight-ray CUDA path;
        ...
        for (int frame = 308; frame < 700; ++frame) {
